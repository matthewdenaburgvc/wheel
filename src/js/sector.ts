...
class Sector {
  ...
  toPolarPoint(angle: Angle): { x: number, y: number } {
  ...
  convertPointToString(point: { x: number, y: number }): string {
  ...
  get clipPath(): string {
    ...
    const arc = [
      ...
      1,
      this.convertPointToString(arcPoint)
    ].join(' ');

    const path = [
      `M ${center}`, // move to center
      `L ${this.convertPointToString(radiusPoint)}`, // line to start point
